...
pbd_Bayes3par = function(brts, # branching times
                         ...
                         prior_mu2, # logLik function for 'mu2' - ext incipient
                         ...
                         upper = c(20, 10, 20), # upper limit for the parameters proposal; can be NULL (ie, NO limit) or a vector of 4 in order = "b", "mu", "la1".
                         rep = 1e+5,
                         file = NULL,
                         ...) {
  ...
  if(is.null(file)){
    ...
    for(i in 1:rep){
      ...
      # constrains the number of parameters to be 3
      # mu1 and mu2 are forced to have the same value
      test1 = "mu1" %in% par
      test2 = "mu2" %in% par
      if(test1 & test2){ # both were sampled
        # uses mu1 as default
        proposal[which(par == "mu2")] = proposal[which(par == "mu1")]
      } else if(test1){ # only mu1
        proposal = c(proposal, proposal[which(par == "mu1")])
        par = c(par, "mu2")
      } else if(test2){ # only mu2
        proposal = c(proposal, proposal[which(par == "mu2")])
        par = c(par, "mu1")
      }
