...
#load("data/pbd_sim_cpp_full.RData")
# load("pbd_sim_cpp_full.RData")
...
rep = 5e+4
chains = 10
files = sapply(1:length(phylos), function(x) paste0("Bayes_test_param_", names(phylos)[x], "_phy_", 1:length(phylos[[x]])))
#source("analysis/R/pbd_Bayes.R")
source("pbd_Bayes.R")
for(i in 1:length(branches)){
  sim.pars = as.numeric(strsplit(names(phylos)[i], "_")[[1]])
