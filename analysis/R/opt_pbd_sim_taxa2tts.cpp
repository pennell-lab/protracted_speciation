...
// [[Rcpp::export]]
Rcpp::NumericMatrix pbdLoop_taxa2tts(Rcpp::NumericVector pars, int taxa, double ntry){
  
  
  double t, denom;
  int id, Sid; //int Sid1 = 0;
  int event, Ng, Ni1, Ni2;
  Rcpp::NumericVector sg, contr, si1, si2, vec, probs;
  ...
  while(ntry > 0){
    id = 2;
    Sid = 2;
    sg = NumericVector::create(NA_REAL, 1, 2);
    contr = NumericVector::create(NA_REAL, 1, 2);
    si1 = NumericVector::create(NA_REAL);
    si2 = NumericVector::create(NA_REAL);
    Ng = sg.size() - 1;
    Ni1 = si1.size() - 1;
    Ni2 = si2.size() - 1;
    Rcpp::NumericMatrix L(2, 5);
    L(0, 0) = 1;
    L(0, 1) = 0;
    L(0, 2) = 0;
    L(0, 3) = 0;
    ...
    while(bb) {
      ...
      case (1): { 
        parent = get_id(sg);
        id += 1;
        vec = NumericVector::create(id, sg(parent), t, -1, -1, L(abs(sg(parent)) - 1, 5));
        L = addRow(L, vec);
        if(contr(parent) == 1){
