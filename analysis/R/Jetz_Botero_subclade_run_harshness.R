...
source(paste0(input_code, "pbd_Bayes.R"))
...
load(file = paste0(input_data, "Jetz_Botero14_subclades_priorb1expHarsh.RData"))
...
lapply(unique(propMixedHarsh$phy), function(i){
  ...
  files = paste0(ouput_data, "Jetz_Botero_subclade_phy_", i,
                 "_harshness_", this_data$which.max,
  ...
  mclapply(ind, FUN = function(xxx){
    for(chn in 1:chains){
      pbd_Bayes(brts = branchesHarsh.unity[[i]][[xxx]],
                initparsopt = rep(chn, 4),
                prior_b = priorb1expHarsh[[i]][[xxx]],
                prior_mu1 = prior_mu1, 
                prior_la1 = prior_la1,
                prior_mu2 = prior_mu2,
